        ...
        <div class="card border-0 shadow-sm mb-3">
            <div class="card-body">
                <div class="d-flex flex-wrap gap-2 mb-3">
                    <button v-for="p in ['day', 'week', 'month', 'year']" :key="p" class="btn btn-sm"
                ...
                <!-- 기간 라벨 (오늘 기분은 숨김) -->
                <div v-if="rangeLabel && period !== 'day'" class="text-secondary small mt-2">기간: {{ rangeLabel }}</div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    <div class="fw-semibold">
                        ...
                        <span v-else-if="aiMeta.status === 'partial-fallback'" class="badge text-bg-warning ms-2">일부 자동
                            가이드</span>
...
function computeRange() {
    ...
    if (period.value === 'week') {
        const r = weekRange(inputs.value.anyDayInWeek); return { start: r.start, end: r.end, apiPeriod: 'weekly' }
    }
    if (period.value === 'month') {
        const r = monthRange(inputs.value.year, inputs.value.month); return { start: r.start, end: r.end, apiPeriod: 'monthly' }
    }
...
onMounted(async () => {
    try {
        ...
        userId.value = me?.userNo ?? me?.id ?? me ?? 4
    } finally {
        // 기본 탭: 오늘 기분 → 자동 갱신 시작
        startDailyAutoRefresh()
