...
public class ConnectController {
	...
	public Map<String, Object> create(@RequestBody(required = false) ConnectVO vo,
		...
		if (!samePatient) {
			...
			try {
				patientIsRole3 = connectDAO.isPatientRole3(patientNo);
			} catch (Exception ignore) {
			}
			...
			if (currentCnt >= maxGuardians) {
				res.put("ok", false);
				res.put("message", patientIsRole3 ? "보호자는 최대 3명까지 연결할 수 있습니다." : "구독이 없는 경우 보호자는 1명까지만 연결할 수 있습니다.");
				return res;
			}
		}
